...
def _get_school(name: str) -> SchoolInfo | None:
    response = globalvars.http_client.post(FIND_SCHOOLS_URL, json={"name": name})
    data = response.json()
...
def login(credentials: Credentials) -> Token | None:
    ...
    response = globalvars.http_client.post(GET_TOKEN_URL, params=payload)
    data = response.json()
    if data.get("success", False) is False:
...
def _get_headers(token: Token) -> dict[str, str] | None:
...
def get_hw_list(token: Token) -> list[HomeworkRecord] | None:
    ...
    while cur_page_index <= max_page_index:
        response = globalvars.http_client.post(
            GET_HW_LIST_URL,
            headers=headers,
...
def _get_hw_details(token: Token, record: HomeworkRecord) -> dict | None:
    headers = _get_headers(token)
    if headers is None:
        print("<error> authorization failed")
        return None
...
def _get_hw_paper(token: Token, record: HomeworkRecord) -> dict | None:
    headers = _get_headers(token)
    if headers is None:
        print("<error> authorization failed")
        return None
...
def _get_questions(token: Token, record: HomeworkRecord) -> list | None:
    headers = _get_headers(token)
    if headers is None:
        print("<error> authorization failed")
        return None
    ...
    q_list: list[dict] = list(
        map(
            lambda q: {
                "index": int(q["questionNumber"]),  # starts from 1
                "api_id": q["id"],
...
def _get_audio_url(token: Token, record: HomeworkRecord) -> str | None:
...
def get_text(token: Token, record: HomeworkRecord) -> str | None:
...
def _get_answers_cache(token: Token, record: HomeworkRecord) -> list[dict] | None:
...
def get_paper_answers(token: Token, record: HomeworkRecord) -> list[dict] | None:
    print(f"--- step: retrieve answers from paper for '{record.title}' ---")
